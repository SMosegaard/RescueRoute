...
```{r}
pacman::p_load(knitr, rmdformats, Rcpp, fasterize, mapboxapi, leaflet, tidyverse, sf, mapview, ggplot2, opencage, mapboxapi)
...
# Forward the geocodings to latitude and longitude tuples and save the
dataframes in.csv format in the 'data' folder.
```{r}
...
#all_hospitals_iso_15 <- read_sf("../data/isochrones/all_hospitals_after_iso_15.shp")
...
pal <- colorNumeric("viridis", all_hospitals_before_continious_iso$time, na.color = "transparent")
...
pal <- colorNumeric("viridis", all_hospitals_after_continious_iso$time, na.color = "transparent")
...
pal <- colorNumeric("viridis", all_bases_continious_iso$time, na.color = "transparent")
...
plot(st_geometry(all_hospitals_before_union_30), add = TRUE, col = "red", alpha = 0.5)
...
plot(st_geometry(municipalities))
plot(st_geometry(all_hospitals_after_union_30), add = TRUE, col = "red", alpha = 0.5)
```
...
plot(st_geometry(municipalities))
plot(st_geometry(all_hospitals_before_union_15), add = TRUE, col = "red", alpha = 0.5)
...
plot(st_geometry(municipalities))
plot(st_geometry(all_hospitals_after_union_15), add = TRUE, col = "red", alpha = 0.5)
```
